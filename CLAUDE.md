...
| **Prossima azione** | **Sessione B**: #17/#16/#18/#19 committati (`379ace4`, `59e56a1`, `4fb21bb`, `30ae3af`). `ci-smoke` era **rosso** (run 33057147479): **T0.6 PASS** (progression test flippato, P-1 chiuso nel comportamento), ma T0.2/T0.3 **FAIL nell'harness, non nel broker** — `t0.02*`/`t0.03*` chiamavano `kafka-topics.sh`/`kafka-console-consumer.sh` per nome nudo, e in `apache/kafka:4.3.1` i binari sono in `/opt/kafka/bin/` senza essere in PATH (verificato su ENV-W con `docker run`). **Fix committato e pushato** da B (path assoluto con fallback al nome nudo, come l'healthcheck del compose) — **da riconfermare su `ci-smoke`**, nessun run reale ancora dopo il fix. **Sessione A**: #41/#42 corretti, pushati e **verificati su ENV-W il 27/08** con una nightly reale — #42 chiuso per misura. **Prossimo passo**: verificare `ci-smoke` sul nuovo push di B; poi un run `full` su ENV-W contro quel commit per T0.6 definitivo e non-regressione T0.2/T0.3; se tutto verde, #20 (gate di release, CHANGELOG → versione, README a fine release, tag) |
