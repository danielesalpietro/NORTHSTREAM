...
| **Prossima azione** | **Gate #20 eseguito da sessione B**: P-13 verificato su ENV-W (protocollo a tre stati + controprova sul clone reale che quella mattina non riusciva a fare `git pull`) — **tutto lo scope di v0.0.2 è ora misurato su hardware reale**. `CHANGELOG.md` promosso a `[v0.0.2] — 2026-08-27` con ogni riga che cita il suo riscontro (test automatico dove la CI copre, misura ENV-W dove non può per costruzione — P-11/P-12/P-13, T0.6 con modelli veri) e la qualificazione esplicita su T0.4/T0.5 (verdi a stack caldo, non ancora una guardia di non-regressione affidabile senza #47); README aggiornato solo per le parti cambiate e coperte da test verde (doppio endpoint Kafka, riproducibilità immagini, nuovo passo preflight), T0.12 verificato verde a ogni modifica. **Nessun tag**: le sessioni cloud prendono 403 dal proxy git sui tag — spetta all'owner, dopo conferma che `ci-static`/`ci-smoke` siano verdi su questo push. **Prossimo passo**: verificare CI verde, poi tag `v0.0.2` (owner) e apertura Fase 2 |
