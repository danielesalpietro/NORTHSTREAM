...
| **Prossima azione** | [#15](https://github.com/danielesalpietro/NORTHSTREAM/issues/15) tag `v0.0.1` — sbloccata: [#10](https://github.com/danielesalpietro/NORTHSTREAM/issues/10) e [#13](https://github.com/danielesalpietro/NORTHSTREAM/issues/13) hanno raggiunto la loro DoD su ENV-W (tabella confermato/smentito e run di riferimento archiviato). Prima del tag: decidere sui punti aperti in fondo alla 6ª entry del logbook |
...
| **Blocchi aperti** | RP-0 (probe DinD su RunPod, issue #34) non ancora eseguito · runner self-hosted ENV-W + variabile `RUN_NIGHTLY` da configurare per attivare `ci-nightly` (issue #12) · **ENV-W non può pubblicare su GitHub** (bloccante): nessuna credenziale HTTPS, `gh` non autenticato, chiave SSH non abilitata → `git push` fallisce con exit 128 e i commenti alle issue non sono postabili. I commit di #10/#13 (`111665a`, `1f77c67` + una correzione) esistono **solo sulla Z8**. Serve `gh auth login` dell'owner sulla Z8, o un push manuale dell'owner · **XPASS di T0.10** nel run di riferimento da sanare prima che venga letto come difetto risolto (v. 6ª entry, decisione 2) · **P-9 proposto**: i tre script del Quick Start sono `100644` in git, quindi `./start-addon.sh` non è eseguibile su un clone pulito · limite settimanale claude-fable-5 esaurito fino al ~28/08 (usare opus-5 o sonnet-5) |
