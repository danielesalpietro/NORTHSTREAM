...
| **Prossima azione** | **A-3 è chiuso** (31/08, ENV-W): id di punto Qdrant da `uuid5(NAMESPACE, "topic:partition:offset")`, **T0.8 XFAIL → PASS** falsificato rimettendo il contatore, suite piena **10 PASS / 2 XPASS / 1 XFAIL** senza regressioni, e prova diretta su tutta la collection attraverso un riavvio con **0 punti preesistenti cambiati e 0 spariti** ([`20260831-1209-envw-db4c22a-a3.md`](docs/runs/20260831-1209-envw-db4c22a-a3.md)). Migrazione: **opzione (1)**, i punti legacy restano — la corruzione storica è congelata nel corpus e la ricreazione della collection si decide in **A-2**. Adesso **[#40](https://github.com/danielesalpietro/NORTHSTREAM/issues/40)** (irrobustire T0.10: XPASS anche il 31/08, **terzo** esito discordante) perché **sblocca A-1**, poi **A-2**, **A-1**, **A-5**. **T-SOAK-24h ed EVAL sono ora sbloccati**: il corpus non viene più sovrascritto |
...
| **Sessioni operative attive** | **A-3** → **ENV-W Z8** `session_011JstTbN3FioNR334Y9ZWnf`, assegnata il 31/08 su `release/v0.0.4`, briefing in [`docs/handoff_per_A3.md`](docs/handoff_per_A3.md). **Cambio di ruolo da registrare**: fino alla Fase 2 questa sessione era host di verifica e *non committava codice* (eccezione: report di run ed entry di logbook); con A-3 committa `stream-agent/`. **Nessun'altra sessione può toccare l'agent finché A-3 è aperto** (§3.8). Si è archiviata **quattro volte in tre giorni**, una a nove minuti da un compito: il campionatore distaccato ha salvato la misura tre volte su tre, il passo di archiviazione è stato saltato ogni volta. Da qui `bench/gate/run_caps_gate.sh` (si distacca e chiude l'archivio da sé) e `bench/env-w/start-session.sh` (avvia in `tmux` e dal repo — il rimedio raccomandato tre volte e mai applicato). **#23 non assegnata**: vuole **ENV-L**, laptop da accendere su richiesta |
| **Blocchi aperti** | **`RUN_NIGHTLY` resta spenta**: resta [#47](https://github.com/danielesalpietro/NORTHSTREAM/issues/47) (warm-up gate) e la decisione è dell'owner · **[#44](https://github.com/danielesalpietro/NORTHSTREAM/issues/44) da riverificare su due device**: la lettura di capacità sommava le schede — con un tenant su 22 GiB della 3090 dichiarava **18 260 MiB liberi** mentre la scheda singola più libera ne aveva **16 184**, e un run che ne chiedeva 17 000 passava il pre-check e moriva al caricamento. Corretto (`e5588c9`), non ancora verificato a runtime · [#40](https://github.com/danielesalpietro/NORTHSTREAM/issues/40) T0.10 non robusto, **colto in flagrante** (XPASS 12:40Z / XFAIL 12:47Z sullo stesso stack): da chiudere in questa fase · **T-SOAK-24h in attesa di A-3** · **`preflight.ps1` mai collaudato su Windows/ENV-L** · RP-0 opzionale ma non archiviabile |
