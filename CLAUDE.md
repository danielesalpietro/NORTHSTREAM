...
| **Prossima azione** | **T-SOAK-24h in corso**, primo del progetto: al termine (01/09 ~16:44Z) verificare **prima l'integrità** dell'archivio (`seq` contigui, `soak.err.log`, `sha256sum -c`) e poi il verdetto sui quattro check di §4.3.1, **calcolando (a) a mano** perché `verdict.py` diverge dal piano. Poi **[#40](https://github.com/danielesalpietro/NORTHSTREAM/issues/40)** (irrobustire T0.10 — terzo esito discordante, e **sblocca A-1**), quindi **A-2**, **A-1** (rimozione del boost keyword), **A-5**. Restano [#23](https://github.com/danielesalpietro/NORTHSTREAM/issues/23) su ENV-L e la **retention del corpus**, orfana dalla chiusura di A-3 e senza progression test |
...
| **Sessioni operative attive** | **T-SOAK-24h** → `session_016PpJwAWbM1rDyfSsZ5boZV` ("Accesso ambiente test_and_dev", opera su ENV-W via SSH dal laptop dell'owner). Run **`20260831-1644-envw-4d5f24a`** partito il 31/08 alle 16:44Z, distaccato sotto PPID 1, fine attesa **01/09 ~16:44Z** con `SHA256SUMS` scritto dal run stesso. **A-3 chiuso** (`8f12bde`) dalla sessione ENV-W `session_011JstTbN3FioNR334Y9ZWnf`, che resta il bridge di verifica. **Nessun'altra sessione tocca l'agent o lo stack su ENV-W finché il soak gira**: un teardown o un riavvio invaliderebbe 24 ore di misura |
