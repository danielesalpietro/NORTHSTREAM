...
| **Ultimo run T0** | [`docs/runs/20260827-1148-envw-6b377a3.md`](docs/runs/20260827-1148-envw-6b377a3.md) — due nightly consecutive su ENV-W in stato **esclusivo** con modelli veri: **T0.6 PASS** in entrambe (`broker 1 at localhost:29092`), T0.2/T0.3 PASS in entrambe. T0.4/T0.5 concludono `failure` **a freddo** e sono verdi a stack caldo: **non regressioni** (A-8/[#39](https://github.com/danielesalpietro/NORTHSTREAM/issues/39)), ma finché [#47](https://github.com/danielesalpietro/NORTHSTREAM/issues/47) è aperto la nightly **non è una guardia di non-regressione affidabile**. Metro invariato: [`docs/runs/20260826-2053-envw-5eb456a-baseline.md`](docs/runs/20260826-2053-envw-5eb456a-baseline.md) — 5 PASS + 6 XFAIL + 1 XPASS. Misura di capacità: [`docs/runs/20260827-1115-envw-c424928-vram32b.md`](docs/runs/20260827-1115-envw-c424928-vram32b.md), `granite4:32b-a9b-h` **entra** nella 3090 (19 788 / 24 576 MiB, 100% GPU). **Soak parziale archiviato** (28/08, `c83bdff` su `feature/soak-harness`): `20260827-1406-envw-c6b56d3`, **427 campioni contigui su 7,39 h**, zero errori, zero warning, slot `active` vero in 427/427 (il fix `2b4f4ad` regge). Verdetto §4.3: (a) WARN atteso (crescita Qdrant, è A-3), (c) OK (divario DB/Qdrant costante entro l'errore di campionamento), (b) e (d) **UNKNOWN** perché il piano non dichiarava le soglie — ora scritte in `docs/piano_ricovero.md` §4.3.1. È il **"prima" di P-5**: RSS mediana 14 174 → picco 15 985 MiB, di cui Trino 4 109 → 7 119 MiB (+73%) da solo, mentre Elasticsearch (`-Xmx1g` fisso) e Kafka restano piatti |
| **Prossima azione** | **T0.7 flippato a PASS su hardware vero** (`7f9e5a9`, suite 9 PASS / 3 XFAIL / 1 XPASS, nessuna regressione) e **#44 verificato su quattro comportamenti del gate**, incluso `unknown` provato nascondendo `nvidia-smi` a contesa viva (risponde `unknown` con campi `null`, non `shared`). **Ma il test T0.7 non poteva fallire**: univa stderr a stdout e ricavava il conteggio da `head -1 | tr -dc '0-9'`, cioè dal timestamp del WARNING di jline — una query a zero righe veniva valutata OK. La sessione ENV-W l'ha dichiarato invece di incassare il verde; correzione in `acc0154`, **da falsificare su ENV-W prima di considerarla buona**. Poi **T-PROF parte 1** (profilo `core`, stack freddo, tempo di misura dichiarato, contro i 14 174 MiB di mediana dello stack pieno senza tetti) e **soak #2 da sei ore con tutti i profili attivi**. Restano [#23](https://github.com/danielesalpietro/NORTHSTREAM/issues/23) (righe VRAM 16 GB e conferma comportamentale, **su ENV-L**) e [#24](https://github.com/danielesalpietro/NORTHSTREAM/issues/24) (release v0.0.3: gate, CHANGELOG, tag) |
| **Finestra ENV-W** | **Estesa dall'owner fino alle ~02:00Z del 29/08** (dieci ore dalle 16:00Z), macchina esclusiva verificata. Piano orario concordato: falsificazione T0.7 → T-PROF parte 1 → **soak #2 da sei ore lanciato ~17:15Z** → archiviazione a ~23:15Z, con due ore di margine per il report. La sessione bridge si è **archiviata da sola alle 13:45Z** a metà finestra (seconda volta: la CLI non gira dentro `tmux`, v. §5) ed è stata riavviata dall'owner alle ~16:00Z **con contesto intatto** — il trigger in coda le è stato consegnato al rientro. **ENV-W ha due stati** — manutenzione (misure GPU valide) e noleggio vast.ai (GPU di terzi) — e le finestre **si prenotano con anticipo**: durate e regole in `docs/piano_ricovero.md` §2.1 |
