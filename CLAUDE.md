...
| **Prossima azione** | **[#22](https://github.com/danielesalpietro/NORTHSTREAM/issues/22) e [#21](https://github.com/danielesalpietro/NORTHSTREAM/issues/21) implementati** (catalogo `postgresql.properties` + heap Trino fisso; profili `core`/`lakehouse`/`governance` + `mem_limit` su 20/21 servizi), **verificati solo staticamente** (sessione senza demone Docker): serve un run reale (ENV-L o nightly ENV-W) per far flippare **T0.7** in `expected/current.json` e per scoprire se un `mem_limit` stimato è troppo stretto — v. `docs/logbook/LOGBOOK_v0.0.3.md`. Poi [#23](https://github.com/danielesalpietro/NORTHSTREAM/issues/23) (tier misurati + T-PROF, **su ENV-L**: su ENV-W il footprint è un limite superiore) e [#44](https://github.com/danielesalpietro/NORTHSTREAM/issues/44) (esclusività dell'host, **in corso**, sessione D, su `feature/soak-harness`: `fe91a74`, `cdde3a7`) |
...
| **Sessioni operative attive** | **Sessione C** `session_01Np7X8P4C4cUQAikUz4FDHP` (`claude-sonnet-5`): **#22 e #21 implementati** (catalogo Trino + memoria, poi profiles e `mem_limit`), **verificati solo staticamente** — nessun demone Docker in questa sessione, serve un run reale per flippare T0.7 e collaudare i `mem_limit` stimati (v. `docs/logbook/LOGBOOK_v0.0.3.md`) · **Sessione D** `session_01HVaaYgFTssQc54oiojLzqQ` (`claude-sonnet-5`): **#44** esclusività dell'host, partendo da `fe91a74`/`cdde3a7` su `feature/soak-harness` invece che da zero. C tocca i compose, D tocca `preflight.*` e `bench/`: si incontrano solo su CHANGELOG, logbook e §2 (§3.8). **#23 non assegnata**: T-PROF va misurato su **ENV-L**, e il laptop è spento — va chiesto all'owner quando è disponibile. Chiuse in Fase 1: **A** `session_01G56RvSh4eQyZuJ7UjZFJ9i` (#41, #42, poi `bench/soak/` e #44 parziale) · **B** `session_01AJY9RuEEEzZ8ZzBazDNmbF` (#17→#16→#18→#19, poi #45/#46/#48 e gate #20) · **ENV-W Z8** `session_011JstTbN3FioNR334Y9ZWnf` (bridge, host di verifica: esegue e riferisce, **non committa codice**) — ancora attiva fino alla chiusura del soak |
| **Blocchi aperti** | **`RUN_NIGHTLY` resta spenta**: il blocco tecnico (#42) è caduto, ma restano [#47](https://github.com/danielesalpietro/NORTHSTREAM/issues/47) (warm-up gate: senza, i FAIL notturni di T0.4/T0.5 sono rumore di condizione) e [#44](https://github.com/danielesalpietro/NORTHSTREAM/issues/44) (alle 02:30 nessuno sa in che stato sia ENV-W). Accenderla è una decisione dell'owner · [#40](https://github.com/danielesalpietro/NORTHSTREAM/issues/40) T0.10 non robusto (Fase 3) · **T-SOAK-24h non eseguito**: solo un parziale da ~7 h; l'harness vive su `feature/soak-harness` e serve a v0.0.4 · **`preflight.ps1` mai collaudato su Windows/ENV-L** · RP-0 opzionale ma **non archiviabile**: è la riserva quando ENV-W è a noleggio |
