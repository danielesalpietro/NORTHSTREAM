...
| **Prossima azione** | Sessione B: **#17 + #16 nello stesso ordine — prima la sostituzione dell'immagine Kafka, poi il doppio listener**. Bitnami e Apache usano prefissi di variabili diversi (`KAFKA_CFG_*` vs `KAFKA_*`): configurare i listener prima della migrazione significa scriverli due volte e far flippare T0.6 su un'immagine che poi cambia. Requisito del briefing: **T0.6 va promosso nella suite `ci`**, altrimenti il progression test della release non gira in CI. Poi #18 e #19, in coda alla stessa sessione (stesso file compose: aprirli in parallelo crea conflitti) |
| **Sessioni operative attive** | **Sessione A** `session_01G56RvSh4eQyZuJ7UjZFJ9i` (`claude-sonnet-5`): **#41 e #42 corretti e pushati** su `release/v0.0.2` (commit `5f98800`, `531ed58`) — turno chiuso lato sessione (CLAUDE.md §7), resta da innescare una nightly reale su ENV-W per il criterio di verifica di #42 · **Sessione B** `session_01AJY9RuEEEzZ8ZzBazDNmbF` (`claude-sonnet-5`): **#17 → #16 → #18 → #19** · **ENV-W Z8** `session_011JstTbN3FioNR334Y9ZWnf` (bridge, host di verifica: esegue e riferisce, **non committa codice**). A e B lavorano in parallelo sullo stesso branch: si incontrano solo su CHANGELOG, logbook e §2, e si coordinano con `git pull --rebase` (§3.8). Le prime A e B (`…01Gu8Zq`, `…01GQuK9`) sono state archiviate senza aver prodotto nulla: v. la lezione in §7 |
| **Blocchi aperti** | **`RUN_NIGHTLY` NON va impostata finché [#42](https://github.com/danielesalpietro/NORTHSTREAM/issues/42) non è chiuso**. **Aggiornamento 27/08 (Sessione A)**: il teardown è stato corretto (`down -v` → `down` + rimozione esplicita dei soli volumi `kafka_data`/`postgres_data`/`qdrant_data`, `ollama_data` preservato), ma il fix è **verificato solo staticamente** (YAML valido, `docker compose config -q` verde); manca ancora **una nightly reale su ENV-W** che confermi che `ollama_data` sopravvive — quello è il progression test dichiarato di #42, non il codice da solo. Il dispatch manuale bypassa comunque il gate `vars.RUN_NIGHTLY`: ora è sicuro farlo (col fix in `release/v0.0.2`), ma va lanciato dalla sessione ENV-W, non da una sessione cloud senza demone Docker. Il runner `z8-env-w` è registrato e attivo · [#40](https://github.com/danielesalpietro/NORTHSTREAM/issues/40) T0.10 non robusto (Fase 3) · P-5 verificabile solo su ENV-L o con `mem_limit` (Fase 2, T-PROF) · RP-0 non eseguito ma **opzionale**: la matrice EVAL entra nella 3090 · limite settimanale `claude-fable-5` esaurito fino al ~28/08 |
