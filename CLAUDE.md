...
| **Sessioni operative attive** | **Sessione A** `session_01G56RvSh4eQyZuJ7UjZFJ9i` (`claude-sonnet-5`): **#41 poi #42** · **Sessione B** `session_01AJY9RuEEEzZ8ZzBazDNmbF` (`claude-sonnet-5`): **#17 → #16 → #18 → #19** · **ENV-W Z8** `session_011JstTbN3FioNR334Y9ZWnf` (bridge, host di verifica: esegue e riferisce, **non committa codice**). A e B lavorano in parallelo sullo stesso branch: si incontrano solo su CHANGELOG, logbook e §2, e si coordinano con `git pull --rebase` (§3.8). L'ordine #41→#42 non è arbitrario: L'ordine non è arbitrario: con P-9 aperto `ci-nightly` muore al primo step (`run: ./start-addon.sh --gpu` su file `100644` dopo `actions/checkout`), quindi la nightly non è mai girata e #42 non sarebbe verificabile. Le prime A e B (`…01Gu8Zq`, `…01GQuK9`) sono state archiviate senza aver prodotto nulla: v. la lezione in §7 |
| **Blocchi aperti** | **`RUN_NIGHTLY` NON va impostata finché [#42](https://github.com/danielesalpietro/NORTHSTREAM/issues/42) non è chiuso**: il teardown di `ci-nightly` fa `down -v` e cancellerebbe `ollama_data` a ogni notte. **Nota aggiunta il 27/08**: il gate di riga 31 è `vars.RUN_NIGHTLY == 'true' || github.event_name == 'workflow_dispatch'` — il **dispatch manuale bypassa il gate**, quindi anche a variabile spenta una nightly lanciata a mano oggi cancellerebbe i modelli. Nessun dispatch prima del fix. Il runner `z8-env-w` è registrato e attivo, ma il gate `if: vars.RUN_NIGHTLY == 'true'` lo tiene dormiente e innocuo · [#40](https://github.com/danielesalpietro/NORTHSTREAM/issues/40) T0.10 non robusto (Fase 3) · P-5 verificabile solo su ENV-L o con `mem_limit` (Fase 2, T-PROF) · RP-0 non eseguito ma **opzionale**: la matrice EVAL entra nella 3090 · limite settimanale `claude-fable-5` esaurito fino al ~28/08 |
