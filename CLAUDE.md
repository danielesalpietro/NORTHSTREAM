...
| **Sessioni operative attive** | Nessuna assegnata in Fase 3. **ENV-W Z8** `session_011JstTbN3FioNR334Y9ZWnf` (bridge, host di verifica: esegue e riferisce, **non committa codice** — eccezione: report di run e entry di logbook) — **si è archiviata quattro volte in tre giorni**, una volta nove minuti dopo aver ricevuto un compito. Il campionatore distaccato ha salvato la misura **tre volte su tre**; il passo di archiviazione è stato saltato ogni volta. Da qui `bench/gate/run_caps_gate.sh`, che si distacca e chiude l'archivio da sé. **`tmux` raccomandato tre volte e mai applicato** (§5). **#23 non assegnata**: vuole **ENV-L**, laptop da accendere su richiesta |
