...
| **Prossima azione** | **Sessione B**: #17/#16/#18/#19 committati (`c33b5b0`, `d00c3ea`, `e7933b4`, `3295c5a`) — verifica solo statica, nessun demone Docker in quella sessione. Prossimo passo: controllare l'esito di `ci-static`/`ci-smoke` sul push (primo collaudo reale del cambio d'immagine Kafka); se rosso, primo sospetto la configurazione KRaft di `apache/kafka` (env var, path healthcheck `/opt/kafka/bin/kafka-topics.sh`, `CLUSTER_ID`) — v. `docs/logbook/LOGBOOK_v0.0.2.md`. **Sessione A**: #41/#42 corretti e pushati (`5f98800`, `531ed58`), turno chiuso lato sessione; resta da innescare una **nightly reale su ENV-W** per il criterio di verifica di #42. Se entrambi verdi: #20 (gate di release, CHANGELOG → versione, README a fine release, tag) |
