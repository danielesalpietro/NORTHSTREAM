...
| **Ultimo run T0** | [`docs/runs/20260827-0859-envw-9082a02.md`](docs/runs/20260827-0859-envw-9082a02.md) — prima nightly reale su ENV-W (`workflow_dispatch`, run 33056266125) contro `9082a02`, cioè **prima** dei commit di B: `{"PASS": 5, "FAIL": 1, "XFAIL": 4, "XPASS": 2}`. **#42 verificato per misura** (`ollama_data` sopravvive al teardown, modelli elencati, blob identici); il FAIL di T0.5 **non è una regressione** ma un artefatto di run a freddo (stesso commit: PASS 3/3 a stack caldo). Metro invariato: [`docs/runs/20260826-2053-envw-5eb456a-baseline.md`](docs/runs/20260826-2053-envw-5eb456a-baseline.md) — **run di riferimento**: suite `full` contro il tag baseline su ENV-W con modelli reali, 5 PASS + 6 XFAIL + 1 XPASS, `RESULT: OK`. È il metro: ogni release si misura contro questa tabella. L'XPASS di T0.10 **non** è un difetto risolto (issue #40) |
| **Prossima azione** | **Sessione B**: #17/#16/#18/#19 committati; il fix del PATH Kafka nell'harness è **confermato**: `ci-smoke` è tornato verde e T0.2/T0.3 sono di nuovo PASS — T0.6 resta il progression test superato. La finestra esclusiva ENV-W ha poi trovato **due nuovi difetti nello scope della release**, entrambi verificati per misura e già implementati da B: **P-11/[#45](https://github.com/danielesalpietro/NORTHSTREAM/issues/45)** (il volume `kafka_data` di un lab esistente non è scrivibile dal nuovo utente `1000:1000` di `apache/kafka` — crash-loop; fix: `preflight.sh`/`.ps1` rileva e rifiuta con rimedio, più riga breaking nel CHANGELOG) e **P-12/[#46](https://github.com/danielesalpietro/NORTHSTREAM/issues/46)** (`trino/catalog` creata `root:root` da Docker blocca `actions/checkout` a ogni nightly successiva alla prima; fix: `trino/catalog/.gitkeep` committato). **Nessuno dei due è verificabile in CI per costruzione** (ogni run parte da uno stato che non riproduce il difetto): la verifica reale di entrambi spetta a ENV-W. **Sessione A**: #41/#42 chiusi e verificati su ENV-W. **Prossimo passo**: ENV-W verifica P-11 (volume creato deliberatamente con la vecchia proprietà) e P-12 (due nightly consecutive, dopo che l'owner ha sbloccato con `sudo` la `trino/catalog` già compromessa sul workspace); se entrambe confermano, #20 (gate di release, CHANGELOG → versione, README a fine release, tag) |
| **Finestra ENV-W** | **Manutenzione attiva dal 27/08 ~10:00Z per 12 h — la Z8 è in esclusiva, estendibile su richiesta all'owner** (confermato il 27/08). È lo stato in cui le misure GPU valgono (§2.1 del piano). In corso: suite `full` contro l'head e verifica per misura che `granite4:32b-a9b-h` entri davvero nei 24 GB della 3090 — assunzione su cui poggia la matrice EVAL di beta1 e mai testata. A seguire, se l'harness `bench/soak/` passa la prova a secco, **soak lungo** con estensione della finestra a ~30 h |
