...
| **Prossima azione** | **Sessione B**: #17/#16/#18/#19 committati (`379ace4`, `59e56a1`, `4fb21bb`, `30ae3af`). `ci-smoke` sul push è **rosso** (run 33057147479) ma **T0.6 è PASS**: il progression test della release è flippato, P-1 chiuso nel comportamento. I due FAIL (T0.2, T0.3) sono **nell'harness, non nel broker**: `bench/t0/tests/t0.02*` e `t0.03*` chiamano `kafka-topics.sh`/`kafka-console-consumer.sh` per nome nudo, e in `apache/kafka:4.3.1` i binari sono in `/opt/kafka/bin/` e non nel PATH (verificato su ENV-W con `docker run`). **Prossimo passo di B**: usare il path assoluto con fallback, come già fa l'healthcheck del compose. **Sessione A**: #41/#42 corretti e pushati (`83b416c`, `5534ef7`) e **verificati su ENV-W il 27/08** con una nightly reale — #42 chiuso per misura. **ENV-W**: prossimo run `full` contro `7364349` per misurare **T0.6** (progression test della release) e verificare che #18 non abbia rotto T0.2/T0.3. Se tutto verde: #20 (gate di release, CHANGELOG → versione, README a fine release, tag) |
