...
| **Gate dei due tetti** | **VERDE** — [`docs/runs/20260829-1936-envw-d2bd3aa-gate.md`](docs/runs/20260829-1936-envw-d2bd3aa-gate.md) (`20260829-1936-envw-d2bd3aa-gate`, 46 campioni su 45 min, stack freddo a tutti e tre i profili su `d2bd3aa`). Le tre condizioni di §4.3.1(d): **`RestartCount` 0→0 su tutti e 19** · `elasticsearch` **82,6%** di 2048m (max 1 691,5 MiB, soglia 1 843) · `open-webui` **654,8 MiB piatto**, 63,9% di 1024m, `healthy`, nessun dente di sega. Totale **7 202 MiB su 16 704 (43%)**, `ollama` esente a 604,5 MiB. **Prima/dopo sullo stesso controllo**: ES 1 529→1 688 MiB e open-webui 138→655 — *il consumo è salito perché prima morivano* (23 e 3 474 riavvii nel soak #2, 40 e 6 151 al teardown). `verdict_caps.py` concorda sui numeri ed è stato falsificato su tre punti, ma ha **due difetti**: non implementa l'esenzione dichiarata (quindi non può mai dire OK) e accetta una serie troncata in silenzio (dimostrato con 2 campioni su 46) |
| **Ultimo run T0** | [`docs/runs/20260828-1247-envw-9e71cd5.md`](docs/runs/20260828-1247-envw-9e71cd5.md) — **T0.7 PASS su hardware vero** (suite 9 PASS / 3 XFAIL / 1 XPASS, nessuna regressione), col test **falsificato in tre casi** dopo la correzione `acc0154`. **T-PROF** ([`20260828-1305-envw-9e71cd5-tprof.md`](docs/runs/20260828-1305-envw-9e71cd5-tprof.md)): profilo `core` a **3,0 GiB**, criterio ≤ 14 GB soddisfatto con **oltre 4×** di margine. **Soak #2 chiuso** (`c34bb7b`): 420 campioni su 26 569 s, finestra comune ≈ entrambi i run interi. **P-5 chiuso nel comportamento** — RSS totale mediano **15 170 → 7 986 MiB (−47%)**, e Trino risponde sulla *pendenza*: per quarti **+694/+630/+183/+130 MiB/h senza tetti** (ancora in salita a 7,4 h) contro **+169/+165/+0,5/+0,3** con i tetti; pendenza sulla seconda metà **+150,0 → +0,5 MiB/h**. Verdetti §4.3.1: **(b) OK** su tutte e quattro le condizioni · **(a) e (c) FAIL** per A-3 che sovrascrive il corpus · **(d) FAIL** non sul totale (44% del budget) ma su `elasticsearch` incollato al 99,5% del proprio tetto per 356 campioni consecutivi |
| **Prossima azione** | **Due `mem_limit` di [#21](https://github.com/danielesalpietro/NORTHSTREAM/issues/21) erano sotto il footprint misurato**, entrambi corretti ma **non ancora verificati a runtime**: `open-webui` 512m → **1024m** (`f9a56fc`, 3 474 riavvii in 7,4 h) ed `elasticsearch` 1536m → **2048m** (`75446b7`, 23 riavvii; il tetto era esattamente heap 1024 + direct 512, zero per non-heap). **Gate di #24 eseguito il 29/08 ed è VERDE** (riga sopra): i due tetti sono stati visti girare, non solo ragionati. **Resta #24: CHANGELOG e tag di v0.0.3.** Prima di mettere il gate in nightly vanno corretti i due difetti di `verdict_caps.py` (esenzione dichiarata non implementata → UNKNOWN perpetuo; serie troncata accettata in silenzio → verde che non ha guardato niente). **T-SOAK-24h resta dopo A-3**: oggi (a) e (c) falliscono per la sovrascrittura del corpus, e rifarlo prima significherebbe spendere 24 h per rimisurare un guasto noto. Restano [#23](https://github.com/danielesalpietro/NORTHSTREAM/issues/23) (righe VRAM 16 GB e conferma comportamentale, **su ENV-L**) e [#24](https://github.com/danielesalpietro/NORTHSTREAM/issues/24) (release v0.0.3: gate, CHANGELOG, tag) |
| **Finestra ENV-W** | **Il gate dei due tetti non ha richiesto una finestra**: guarda cgroup e `RestartCount`, non la GPU, e ha girato con l'host non esclusivo (un container estraneo effimero, `load1` a 31,1 per sette minuti — nessuno dei 19 si è mosso di un MiB). **La sessione bridge è morta una quarta volta**, due minuti dopo aver lanciato il gate: il campionatore distaccato ha salvato la misura **tre volte su tre**, ma l'analisi è arrivata il giorno dopo. `tmux` resta non applicato (§5). Finestra GPU precedente **scaduta alle 02:00Z del 29/08.** La finestra 16:00Z–02:00Z ha prodotto T0.7, #44, T-PROF e il soak #2 completo. Serve **una nuova finestra breve (~1 h)** per il gate dei due tetti corretti: da chiedere all'owner. La sessione bridge si è archiviata **tre volte** in due giorni (13:45Z del 28/08, e di nuovo a fine soak): il campionatore distaccato ha salvato la misura **due volte su due**, ma l'analisi è arrivata con un giorno di ritardo. `tmux` resta non applicato (§5). **ENV-W ha due stati** — manutenzione (misure GPU valide) e noleggio vast.ai — e le finestre **si prenotano con anticipo**: `docs/piano_ricovero.md` §2.1 |
| **Sessioni operative attive** | **ENV-W Z8** `session_011JstTbN3FioNR334Y9ZWnf` (bridge, host di verifica: esegue e riferisce, **non committa codice** — eccezione concordata: i report di run e le entry di logbook su `feature/soak-harness`) — **riattivata il 28/08**, sta lavorando la sequenza T0.7 → #44 → T-PROF → soak #2. Sospese il 28/08 per esaurimento crediti e poi riprese: la sospensione ha lasciato un campionatore distaccato in esecuzione e dati non analizzati per ~15 h — v. la lezione in `LOGBOOK_v0.0.3.md`. **#23 non assegnata**: le righe VRAM 16 GB e la conferma comportamentale vogliono **ENV-L**, e il laptop va acceso su richiesta. Chiuse in Fase 1: **A** `session_01G56RvSh4eQyZuJ7UjZFJ9i` (#41, #42, poi `bench/soak/` e #44 parziale) · **B** `session_01AJY9RuEEEzZ8ZzBazDNmbF` (#17→#16→#18→#19, poi #45/#46/#48 e gate #20) |
| **Blocchi aperti** | **`RUN_NIGHTLY` resta spenta**: il blocco tecnico (#42) è caduto, [#44](https://github.com/danielesalpietro/NORTHSTREAM/issues/44) **riverificato il 30/08 su due schede** ([`20260830-2231-envw-d48ed60-44-due-gpu.md`](docs/runs/20260830-2231-envw-d48ed60-44-due-gpu.md)): **lo stato a tre valori regge** — contesa sulla sola 5060 Ti con la 3090 libera → `shared`, perché l'attribuzione passa dal cgroup e non dal device — ma **`gpu_free_mib` è una somma e produce un falso verde**: libero reale 6 437 + 13 657, aggregato 21 011, e `preflight --require-vram-mib 16000` passa con EXIT=0 mentre nessuna scheda può ospitare quel run (e il falso verde **non richiede** `--allow-contention`). Serve la lettura **per device** e la decisione sul massimo di una singola scheda. **Inoltre: Ollama gira 100% CPU dal riavvio del 30/08** — `llama-server --list-devices` → `(none)`, mentre `nvidia-smi -L` dentro il container vede entrambe le schede e un probe torch crea il contesto CUDA in 0,1 s su entrambe: è Ollama, non l'host. ENV-W ha due schede (RTX 3090 24 GB + RTX 5060 Ti 16 GB) e `gpu_memory_totals()` **somma** used e total su tutti i device — un aggregato direbbe "libera" con una delle due occupata. La verifica a tre stati del 28/08 vale per una scheda sola e **va rifatta su due device**; ogni misura VRAM da qui in poi fissa il device e lo scrive nel manifest, e resta [#47](https://github.com/danielesalpietro/NORTHSTREAM/issues/47) (warm-up gate: senza, i FAIL notturni di T0.4/T0.5 sono rumore di condizione). Accenderla è una decisione dell'owner · [#40](https://github.com/danielesalpietro/NORTHSTREAM/issues/40) T0.10 non robusto (Fase 3) · **T-SOAK-24h non eseguito**: il parziale da 7,39 h è archiviato e ha già ripagato la finestra (ha visto la crescita di Trino, che nessuna suite da quindici minuti può vedere), ma le 24 h piene restano da prenotare a parte — harness su `feature/soak-harness`, scope v0.0.4 · **`preflight.ps1` mai collaudato su Windows/ENV-L** (anche per i nuovi flag `-RequireVramMib`/`-RequireRamMib`/`-AllowContention` di #44) · RP-0 opzionale ma **non archiviabile**: è la riserva quando ENV-W è a noleggio |
