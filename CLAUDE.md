...
| **Prossima azione** | **Finestra ENV-W aperta fino alle ~22:00Z del 28/08** e sessione bridge riattivata: l'ordine dei lavori è **(1) T0.7** — progression test dichiarato di v0.0.3, senza il quale la release non chiude — **(2) verifica di [#44](https://github.com/danielesalpietro/NORTHSTREAM/issues/44)** col protocollo a tre stati (libera / contesa deliberata / risanata, come per P-11), **(3) T-PROF parte 1** sul profilo `core` a stack freddo con il tempo di misura dichiarato, **(4) soak #2 solo alla fine**. L'ordine è dettato dall'interferenza: #44 occupa VRAM di proposito e T-PROF cambia la composizione dello stack, quindi eseguirle *durante* il soak #2 ne contaminerebbe la serie. #22/#21/#44 sono implementati ma **verificati solo staticamente**: catalogo `postgresql.properties` + heap Trino fisso; profili `core`/`lakehouse`/`governance` + `mem_limit` su 20/21 servizi; `bench/lib/gpu_exclusivity.py` + `preflight.{sh,ps1} --gpu --require-vram-mib/--require-ram-mib/--allow-contention` + `bench/t0/run.sh --exclusivity` con `manifest.json.exclusivity.{declared,detected}`. Poi [#23](https://github.com/danielesalpietro/NORTHSTREAM/issues/23) (righe VRAM 16 GB e conferma comportamentale, **su ENV-L**) e [#24](https://github.com/danielesalpietro/NORTHSTREAM/issues/24) (release v0.0.3: gate, CHANGELOG, tag) |
| **Finestra ENV-W** | **Manutenzione estesa: la Z8 è nostra fino alle ~22:00Z del 28/08**, macchina esclusiva verificata (`nvidia-smi` 362 MiB / 24 576, 0% util, nessun tenant vast.ai). La finestra del 27/08 si è chiusa alle ~08:15Z col soak #1 completato e archiviato. **ENV-W ha due stati** — manutenzione (misure GPU valide) e noleggio vast.ai (GPU di terzi) — e le finestre **si prenotano con anticipo**: durate e regole in `docs/piano_ricovero.md` §2.1, fra cui la prova a secco prima di ogni finestra prenotata |
