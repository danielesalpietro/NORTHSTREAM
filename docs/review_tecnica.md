...
> **Stato di verifica dei finding sull'agent (aggiornato 26/08/2026).** A-3 e A-5, dichiarati nella §6 come deduzioni statiche da confermare, sono ora **confermati da misura**: run `ci-smoke-33008193653` — T0.8 *"count grew by only 0 after restart: points are being overwritten"* e T0.11 *"/health still reports ok with qdrant down"*. Gli altri finding restano da confermare col run T0 su ENV-W (#13).
